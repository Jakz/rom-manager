...
public class RomSetManagerView extends JPanel
{
  ...
  private class DatTableModel extends AbstractTableModel
  {
    ...
    {
      return c == 0 && data.get(r).isPresentOnDisk();
    }
  ...
  class SystemRomSetInfo extends JPanel
  {
    ...
    SystemRomSetInfo()
    {
      ...
      datTable.getSelectionModel().addListSelectionListener(
          SimpleListSelectionListener.of(index -> info.updateFields(datModel.data.get(index))));
       ...
       * datTable.getDefaultRenderer(String.class);
       * 
       * Predicate<RomSet> predicate = r -> r.canBeLoaded(); Consumer<JComponent>
       * trueEffect = c -> c.setForeground(Color.GREEN); Consumer<JComponent>
       * falseEffect = c -> c.setForeground(UIManager.getColor("Table.foreground"));
       * 
       * datTable.setDefaultRenderer(String.class, new
       * LambdaTableCellRenderer<RomSet>(predicate, trueEffect, falseEffect,
       * stringRenderer)); }
       */
      ...
      if (set != null)
      {
        ...
        try
        {
          ...
          if (isPresent)
          {
            ...
            status.setText("Status: PRESENT ("
                + format.format(new Date(Files.getLastModifiedTime(set.datPath()).toMillis())) + ")");
